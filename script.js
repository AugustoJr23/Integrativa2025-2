...
// ===== SOCKET.IO - INÍCIO DA CONFIGURAÇÃO =====

// Conecta ao servidor Socket.IO (usando a URL do backend)
// NOTA: Se o seu backend estiver noutra URL, ajuste aqui.
// Como o frontend e backend estão no mesmo domínio (servidos pelo mesmo app Express),
// podemos omitir a URL e ele conecta automaticamente.
const socket = io();

// Assim que conectar, avisa o servidor que é um atendente
socket.on('connect', () => {
    console.log('Socket.IO conectado ao servidor com ID:', socket.id);

    // **ESTA É A LINHA CRÍTICA QUE FALTAVA**
    // Entra na sala "atendentes" para receber as mensagens
    socket.emit('join-atendente');
});

// Ouve por novas mensagens enviadas pelo servidor
socket.on('nova-mensagem', (mensagemData) => {
    console.log('Nova mensagem recebida via Socket.IO:', mensagemData);

    // TODO: Chamar a função para renderizar a mensagem no chat
    // Exemplo: renderizarNovaMensagem(mensagemData);

    // Apenas para teste, vamos adicionar ao chat de 'maria-silva'
    if (currentClient === 'maria-silva') {
        adicionarMensagemAoChat(mensagemData);
    } else {
        // TODO: Adicionar um indicador de nova mensagem para outros clientes
        console.log(`Nova mensagem para cliente ${mensagemData.clientId || 'desconhecido'}, mas não é o cliente ativo.`);
    }
});

// Ouve por confirmação de entrada na sala (opcional, para debug)
...
// Função para adicionar uma nova mensagem recebida ao chat
function adicionarMensagemAoChat(mensagemData) {
    const chatMessages = document.querySelector('.chat-messages');
    if (!chatMessages) return; // Sai se o chat não estiver visível

    const config = channelConfig[mensagemData.channel || 'whatsapp']; // Assume WhatsApp se não especificado

    const messageDiv = document.createElement('div');
    // Assume que mensagens via webhook são sempre do cliente (não 'sent')
    messageDiv.className = 'message';

    // Adapte o conteúdo com base na estrutura real de 'mensagemData' vinda do webhook
    // O log que vimos mostra a mensagem dentro de 'mensagemData.content.data.message'
    let displayMessage = 'Mensagem não encontrada na estrutura esperada.';
    try {
        // Tenta extrair a mensagem da estrutura do webhook da Evolution API
        if (mensagemData.content && mensagemData.content.data && mensagemData.content.data.message) {
            // Verifica se é uma mensagem de texto simples
            if (mensagemData.content.data.message.conversation) {
                displayMessage = mensagemData.content.data.message.conversation;
            } else if (mensagemData.content.data.message.extendedTextMessage && mensagemData.content.data.message.extendedTextMessage.text) {
                displayMessage = mensagemData.content.data.message.extendedTextMessage.text;
            } else {
                // Outros tipos de mensagem (imagem, etc.) - Ajustar conforme necessário
                displayMessage = `[${mensagemData.content.data.type || 'Tipo de mensagem não suportado'}]`;
            }
        } else if (mensagemData.content) {
            // Fallback se a estrutura for diferente
            displayMessage = JSON.stringify(mensagemData.content);
        }
    } catch (e) {
        console.error("Erro ao processar conteúdo da mensagem:", e, mensagemData);
        displayMessage = "[Erro ao processar mensagem]";
    }
    ...
    // TODO: Atualizar preview na barra lateral
    // updateConversationPreview(clientId, displayMessage, mensagemData.channel);
}

// ===== SOCKET.IO - FIM DA CONFIGURAÇÃO =====
...
function sendMessage() {
    ...
    if (message) {
        ...
        // Enviar mensagem via Evolution API se canal for WhatsApp
        if (selectedChannel === 'whatsapp') {
            // Número do destinatário (exemplo: do painel ou cliente)
            const toNumber = client.phone.replace(/\D/g, ''); // Remove caracteres não numéricos
